...
## Definition
CodeQL is GitHub's static-analysis engine that scans source for security vulnerabilities — SQL injection, unsafe deserialization, leaked secrets — as an automated step in the [[CI-CD Pipeline]]. It analyzes code without executing it, treating the codebase as a database it can query for dangerous patterns. It's a security-focused sibling of the [[Code Coverage Gate]]: both are machine gates that can fail the build, embodying [[Fail Fast Fail Loudly]] by catching problems in CI rather than in production. It complements human [[Pull Request]] review and [[Code Rabbit]] rather than replacing them.

## Source
GitHub; based on Semmle's QL query language (originally developed by Semmle, acquired by GitHub in 2019). CodeQL released publicly on GitHub Advanced Security.
