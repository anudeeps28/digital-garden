...
## Definition
Traditional logging writes free-form strings: `"Error processing document plan.pdf for group 123"`. Structured logging preserves the data as properties: `{Event: "ProcessingError", FileName: "plan.pdf", GroupNumber: 123, Error: "timeout"}`. This means you can later query logs like "show me all errors for group 123" or "find all timeout errors in the last hour." In the KBA project, [[Serilog]] produces structured logs that are sent to [[Application Insights]], where they're queryable using Kusto Query Language (KQL). This is essential for debugging production issues in a complex pipeline spanning [[Azure Functions]], [[Azure Blob Storage]], and [[Azure OpenAI]].
