...
## Definition
A [[Git]] branch is a lightweight pointer to a series of commits that diverges from the main line of development. When you create a branch (e.g., `feature/add-upload-endpoint`), you get your own workspace where you can make changes without affecting `main`. Other developers can work on their own branches simultaneously. When your feature is ready, you create a [[Pull Request]] to merge your branch back into main. In practice, teams typically follow a branch-per-feature workflow: each story or bug fix gets its own branch, is reviewed via PR, and then merged.

## Source
Linus Torvalds, Git (initial release 2005). Branches are a foundational feature of the Git version control system, designed to enable parallel development workflows.
